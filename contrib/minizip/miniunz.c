...
/*
 * Neutralize absolute paths and Zip-Slip traversal sequences in archive
 * member names (CVE-2014-9485 and incomplete follow-ups).
 *
 * Returns a pointer into |path| suitable for create/open relative to the
 * extraction directory, or NULL if nothing remains after neutralization.
 *
 * Must be applied to BOTH regular files and directory-only entries; the
 * original CVE-2014-9485 mitigation only covered the file branch.
 *
 * Policy matches madler/zlib#908 (remap after the last ".." component) but
 * is component-aware so legitimate names like ".config/" are preserved.
 */
static const char* neutralize_extract_path(const char* path) {
    const char* p;
    const char* result;
    ...
    /* Drop redundant "./" components only (do not strip ".hidden"). */
...
static int do_extract_currentfile(unzFile uf, const int* popt_extract_without_path, int* popt_overwrite, const char* password) {
    ...
    if ((*filename_withoutpath)=='\0')
    {
        ...
        {
            const char* dir_filename = neutralize_extract_path(filename_inzip);
            if (dir_filename != NULL)
            {
                printf("creating directory: %s\n", dir_filename);
                /* Use makedir so nested relative dirs work after
                 * neutralization strips leading "../" prefixes. */
                makedir(dir_filename);
            }
            else
            {
                printf("skipping unsafe directory entry: %s\n", filename_inzip);
            }
        }
    }
    else
    {
        ...
        write_filename = neutralize_extract_path(write_filename);
        if (write_filename == NULL)
        {
            printf("skipping unsafe file entry: %s\n", filename_inzip);
            free(buf);
            return UNZ_OK;
        }
