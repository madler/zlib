...
#include <assert.h>
#define z_assert_str(s) #s
#define z_static_assert(c,m) static_assert(c,z_assert_str(m))
#else
/*
   Parameter Block for Query Available Functions.
 */
#define z_static_assert(c, msg) \
    __attribute__((unused)) \
    static char static_assert_failed_ ## msg[c ? 1 : -1]
#endif
#ifdef HAVE_SECURE_GETENV
#define z_getenv(n) secure_getenv(n)
#else
#define z_getenv(n) getenv(n)
#endif
...
z_static_assert(sizeof(struct dfltcc_qaf_param) == 32,
...
z_static_assert(sizeof(struct dfltcc_param_v0) == 1536,
...
__attribute__((constructor)) local void dfltcc_init_globals(void)
{
    const char *env;
    ...
    env = z_getenv("DFLTCC");
    if (env)
        env_dfltcc_disabled =  !strcmp(env, "0");
    else 
        env_dfltcc_disabled = DFLTCC_DISABLED;

    env = z_getenv("SOURCE_DATE_EPOCH");
    if(env)
        env_dfltcc_source_date_epoch = !strcmp(env, "0");
    else
        env_dfltcc_source_date_epoch = DFLTCC_SOURCE_DATE_EPOCH;


    env_dfltcc_level_mask = xstrtoul(z_getenv("DFLTCC_LEVEL_MASK"),
                              DFLTCC_LEVEL_MASK);

    env_dfltcc_block_size = xstrtoul(z_getenv("DFLTCC_BLOCK_SIZE"),
                              DFLTCC_BLOCK_SIZE);

    env_dfltcc_block_threshold = xstrtoul(z_getenv("DFLTCC_FIRST_FHT_BLOCK_SIZE"),
                                   DFLTCC_BLOCK_THRESHOLD);

    env_dfltcc_dht_threshold = xstrtoul(z_getenv("DFLTCC_DHT_MIN_SAMPLE_SIZE"),
                                 DFLTCC_DHT_THRESHOLD);

    env_dfltcc_ribm = xstrtoul(z_getenv("DFLTCC_RIBM"), DFLTCC_RIBM);
...
local void get_history(struct dfltcc_param_v0 *param,
		       ...
		       Bytef *buf)
{
    if (param->ho + param->hl <= (uint16_t) HB_SIZE)
        /* Circular history buffer does not wrap - copy one chunk */
